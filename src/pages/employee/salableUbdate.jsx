...
import SalableModal from "../../models/employee/BoxUpdate/SalableUpdate";
import { useAuth } from "../../contexts/AuthContext";
import { deleteSalable, getSalable } from "../../service/employee/boxUpdate";
...
const SalableUpdate = () => {
  ...
  useEffect(() => {
    const fetchEpsData = async () => {
      try {
        ...
        const res = await getSalable(auth.company.id, item.customerId, item.boxNo);
        ...
        if (res?.data?.success) {
          ...
          const transformedMaterials = epsData.map(eps => ({
            ...
            salable: eps.salable || 1, // ✅ Include salable field
  ...
  const handleSaveMaterial = (newMaterial) => {
    ...
    if (isEdit) {
      ...
      updated[editIndex] = { ...newMaterial, salable: 1 }; // ✅ Ensure salable is 1
      ...
      setMaterials([...materials, { ...newMaterial, salable: 1 }]); // ✅ Ensure salable is 1
    }
  };
  ...
  return (
    <div className="p-6 bg-white">
      ...
      <div className="pb-2 mb-4">
        <h1 className="text-xl font-bold text-gray-400">SALABLE UPDATE</h1>
      </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {showAddMaterialButton && (
          <button
            onClick={openModal}
            className="px-4 py-2 border text-white border-gray-800 rounded-lg bg-[#6a1a13] hover:bg-[#865556] font-medium"
          >
            Add Material
          </button>
        )}
