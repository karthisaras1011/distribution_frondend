...
import { getEbsReport, exportExcel, exportPDF } from "../../service/employee/epsReport";
...
const EbsReport = () => {
  ...
  }, [form.startDate, form.endDate, form.boxNo, form.recordsPerPage, form.searchTerm]);
  ...
  const fetchData = async (page = 1) => {
    ...
    try {
      ...
      const response = await getEbsReport(params);
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `ebs-report-${new Date().toISOString().split("T")[0]}.xlsx`
      );
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/pdf')) {
        ...
        const filename = `ebs-report-box-${form.boxNo}-${new Date().toISOString().split('T')[0]}.pdf`;
          ...
          <div className="overflow-x-auto border border-gray-200 rounded-lg">
            <table className="min-w-full divide-y divide-gray-200 text-[11px]">
              <thead className=" bg-gray-100 text-gray-600 uppercase text-xs">
                <tr>
                  <th className="py-2 px-1 uppercase tracking-wider ">S.No</th>
                  <th className="py-2 px-1 uppercase tracking-wider">CREATED</th>
                  <th className="py-2 px-1 uppercase tracking-wider">COMPANY NAME</th>
                  <th className="py-2 px-1 uppercase tracking-wider">CUSTOMER NAME</th>
                  <th className="py-2 px-1 uppercase tracking-wider">CUSTOMER CITY</th>
                  <th className="py-2 px-1 uppercase tracking-wider">MATERIAL</th>
                  <th className="py-2 px-1 uppercase tracking-wider">DESCRIPTION</th>
                  <th className="py-2 px-1 uppercase tracking-wider">BATCH NO</th>
                   <th className="py-2 px-1 uppercase tracking-wider">BOX NO</th>
                  <th className="py-2 px-1 uppercase tracking-wider">EXPIRY</th>
                  <th className="py-2 px-1 uppercase tracking-wider">QUANTITY</th>
                  <th className="py-2 px-1 uppercase tracking-wider">FREE QUANTITY</th>
                  <th className="py-2 px-1 uppercase tracking-wider">CONDITION</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {reportData.map((item, index) => (
                  <tr key={item.id || index} className="hover:bg-gray-50">
                    <td className="px-2 py-2 text-center text-[16px]">{index + 1}</td>
                    <td className="px-2 py-2 text-center text-[16px]">
                      {item.created_date ? new Date(item.created_date).toLocaleDateString("en-GB") : "-"}
                    </td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.company_name || "-"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.customer_name || "-"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.customer_city || "-"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.material || "-"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.description || "-"}</td>
                
                    <td className="px-2 py-2 text-center text-[16px]">{item.batch_no || "-"}</td>
                     <td className="px-2 py-2 text-center text-[16px]">{item.box_no || "-"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.expiry_date || "-"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.qty || "0"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">{item.free_qty || "0"}</td>
                    <td className="px-2 py-2 text-center text-[16px]">
